...
class APITest(APITestCase):
    def setUp(self):
        ...
        self.book_subcategory = BookCategory.objects.create(name='Biology', parent=self.book_top_category)
        self.dissertation_top_category = DissertationCategory.objects.create(name='Academic')
        self.dissertation_subcategory = DissertationCategory.objects.create(name='Physics', parent=self.dissertation_top_category)
    ...
    def test_dissertation_category_filter_by_parent(self):
        url = reverse('dissertationcategory-list') + f'?parent={self.dissertation_top_category.id}'
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data['results']), 1)
