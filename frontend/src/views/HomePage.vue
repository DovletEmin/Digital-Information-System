  ...
  <div class="min-h-screen">
    ...
    <section class="text-center py-8 mt-12">
      ...
      <div class="flex justify-center gap-4 mb-12 px-4">
        <router-link
          to="/"
          class="px-6 py-2.5 bg-gray-100 text-gray-900 text-sm font-medium rounded-full hover:bg-gray-200 transition"
        >
          Makalalar
        </router-link>
        <router-link
    ...
    <section class="max-w-6xl mx-auto px-4 mb-16">
      <div class="mb-6">
        <h3 class="text-lg font-medium text-gray-900 mb-4">Kategoriýalar</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-x-24 gap-y-3">
          <div v-for="category in categories" :key="category.id">
            <router-link
              ...
              class="text-sm text-gray-700 hover:text-primary-600 transition"
            >
              {{ category.name }}
            </router-link>
          </div>
          ...
          <div class="flex justify-between items-start">
            <div class="flex-1">
              <div class="flex items-center gap-2 text-xs text-gray-500 mb-2">
                ...
                <span>•</span>
                <span>{{
                  article.institution || "Türkmenistanyň Ylymlar akademiýasy"
                }}</span>
              </div>

              <router-link :to="`/articles/${article.id}`" class="block mb-2">
                <h4
...
const articles = computed(() => contentStore.articles.slice(0, 10));
...
onMounted(async () => {
  ...
  console.log("Categories loaded:", contentStore.articleCategories);
  await contentStore.fetchArticles({ ordering: "-views", page_size: 10 });
  console.log("Articles loaded:", contentStore.articles);
});
</script>
