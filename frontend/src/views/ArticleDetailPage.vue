<template>
  <div class="container mx-auto px-4 py-8">
      ...
      <div class="bg-white rounded-lg shadow-sm p-8 mb-6">
        ...
        <div class="flex items-center justify-between mb-6 pb-6 border-b">
          <div class="space-y-2">
            ...
            <p v-if="article.author_workplace" class="text-gray-600 text-sm">
              {{ article.author_workplace }}
            </p>
        ...
        <div class="flex items-center space-x-6 text-sm text-gray-600 mb-6">
          <div class="flex items-center space-x-2">
            ...
            <span>{{ article.views }} {{ $t("content.views") }}</span>
          </div>

          <div
            v-if="article.average_rating"
    ...
    <div v-else class="text-center py-12">
      <p class="text-gray-600">{{ $t("content.articleNotFound") }}</p>
    </div>
  </div>
</template>
...
const handleBookmark = async () => {
  await bookmarkStore.toggleBookmark(article.value.id, "article");
  article.value.is_bookmarked = !article.value.is_bookmarked;
};

const handleRate = async (rating) => {
  try {
    await ratingService.rateContent(article.value.id, "article", rating);
    userRating.value = rating;

    // Reload article to get updated rating
    const updated = await articleService.getArticle(route.params.id);
    article.value.average_rating = updated.average_rating;
    article.value.rating_count = updated.rating_count;
  } catch (error) {
...
onMounted(async () => {
  try {
    article.value = await articleService.getArticle(route.params.id);
    await articleService.registerView(route.params.id);

    if (authStore.isAuthenticated) {
      userRating.value =
        (await ratingService.getUserRating(article.value.id, "article")) || 0;
    }
  } catch (error) {
