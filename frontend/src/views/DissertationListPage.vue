  ...
  <div class="min-h-screen">
    ...
    <div class="container mx-auto px-4 pb-8">
      ...
      <div class="mb-16 max-w-6xl mx-auto">
        ...
        <!-- Dissertations list -->
        <div v-else-if="dissertations.length > 0" class="space-y-4">
          <article
            v-for="dissertation in dissertations"
            :key="dissertation.id"
            ...
            <div class="flex justify-between items-start">
              <div class="flex-1">
                <div class="flex items-center gap-2 text-xs text-gray-500 mb-2">
                  <span>{{
                    dissertation.categories?.[0]?.name || "Habarlar"
                  }}</span>
                  <span>•</span>
                  <span>{{
                ...
                <p class="text-sm text-gray-600 mb-3 line-clamp-2">
                  {{
                    ...
                    dissertation.content?.substring(0, 200)
                  }}
                </p>
              ...
              <div v-if="dissertation.image" class="ml-6 flex-shrink-0">
                <img
                  :src="dissertation.image"
...
const dissertations = computed(() => contentStore.dissertations);
const categories = computed(() => contentStore.dissertationCategories);
const loading = computed(() => contentStore.loading);
const totalPages = computed(() => {
  const perPage = 12;
  return Math.ceil(contentStore.pagination.dissertations.total / perPage);
});
...
const loadDissertations = async () => {
  ...
  await contentStore.fetchDissertations(params);
};
...
const handleBookmark = async (dissertationId) => {
  await bookmarkStore.toggleBookmark(dissertationId, "dissertation");
...
watch(currentPage, loadDissertations);

onMounted(async () => {
  await contentStore.fetchDissertationCategories();
  await loadDissertations();
});
</script>
